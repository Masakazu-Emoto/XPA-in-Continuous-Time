function [values, vtime, gdot, vvalues, gg1, Kpath] = simulate(g1,impact,T,N,shocks,method,blowup,subset)
...
% Preallocation
[nvars,~] = size(g1);
values = zeros(nvars,N); vvalues = zeros(4*I,N);
if (method == 'implicit')     
    ...
    % For Den Haan Error
    vdot(:,1) = values(1:2*I,1); gdot(:,1) = values(2*I+1:4*I,1); TFP(1,1) = values(4*I,1);
    vvalues(:,1) = [vdot(:,1); gdot(1:end-1,1); TFP(1,1)];
    for n = 1 : N
%         vvalues(:,n) = [vdot(:,n); gdot(1:end-1,n); TFP(1,n)];
        ...
        Kpath(1,n) = K;
        r = exp(Z) * aalpha * (K ^ (aalpha - 1)) * (zAvg ^ (1 - aalpha)) - ddelta;
        w = exp(Z) * (1 - aalpha) * (K ^ aalpha) * (zAvg ^ (-aalpha)); 
        ...
        gdot(:,n+1) = gdot(:,n) + A' * [g; g_end]*dt;
        vdot(:,n+1) =  gg1(1:2*I,:)*(vvalues(:,n) + (dt^(1/2))*impact*shocks(:,n));
        TFP(:,n+1) =  gg1(4*I,:)*(vvalues(:,n) + (dt^(1/2))*impact*shocks(:,n));
        vvalues(:,n+1) = [vdot(:,n+1); gdot(1:end-1,n+1); TFP(1,n+1)];
    end
%     vvalues = vvalues(:,2:end); Kpath = Kpath(2:end);
%     
%     values = values(:,2:end);
    if nargin > 6
