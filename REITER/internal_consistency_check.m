% function [epsilon, Kfull_path, Kred_path, p, p_red, g] = internal_consistency_check(G1,impact,n_g_red,from_red,to_red,g1,psi,F,n_v,n_g,T,steadystate,plotting,IRF,dt)
function [epsilon, Kfull_path, Kred_path, wdist, p, vvec, AA] = internal_consistency_check(G1,impact,n_g_red,from_red,to_red,g1,psi,F,n_v,n_g,T,steadystate,plotting,IRF,dt,vAggregateShock)
...
%     shock = randn(size(impact,2),N+1);
    shock = vAggregateShock;
end

% g_red = impact;
% g = from_red(n_v+1:n_v+n_g,:)*impact;
...
for i = 1:N
    ...
    g_red = g_red + dt*G1*g_red + (dt^(1/2))*impact*shock(:,i+1); % g_red 400*1
    g = g + dt*A(g) + (dt^(1/2))*psi(n_v+1:n_v+n_g)*shock(:,i+1); % Full distribution g 200*1
    wdist(:,i) = g;
    AA = A(g);
    if i == 100
        timed = toc(tstart);
...
disp('The size of transition matrix')
disp(size(A(g)));

epsilon = max(abs(p_red-p)./abs(p),[],3);
Kfull_path = reshape(p(1,:,:),N,1); % the nonlinear dynamics of the full distribution
Kred_path = reshape(p_red(1,:,:),N,1);
