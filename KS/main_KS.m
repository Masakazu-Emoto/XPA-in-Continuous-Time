...
% This code solves the Krusell and Smith model in continuous time by KS Algorithm
% KS algoithm's code is cited by Villaverde et al. (2019 NBER)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Step 2-1 : Inner Loop Taking as given the forecasting rule, calculating policy function
% Step 2-2 : Simulate the path of aggregate capital
% Step 2-3 : Solve for law of motion : Linear Regression
% (Step 3 : Solve for stochastic steady state)
% Step 4 : Simulation the model and Calculate the Den Haan Error
% Step 5 : Plot relevant values
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while (epsilon > epsmin)
    ...
    % Step 2-1 : Inner Loop Taking as given the forecasting rule, calculating policy function
    % -------------------------------------------------- %
    [Ass, Bss, WW, vss, cs, ps] = inner(Kdot, vss, iteration, r, w);
    disp('Finished calculation HJB Equation')
    
    % -------------------------------------------------- %
    % Step 2-2 : Simulate the path of aggregate capital
    % -------------------------------------------------- %
    [Ksim, simZ, Zdown, Zup, Kdown, Kup]  = fokker_planck(Zshocks, muini, Bss);
...
N = 10000; muini = gds; simZ = zeros(N,1); rng(100);  
...
simTFP = exp(simZ);
[sim_mu, KS_K, KS_KK] =  simulate(simZ, muini, Bss, Kdotnew);
