...
if (~savetime)
    ...
    if (loadtemp)
        ...
        disp(' ');
        if (mu==0.25)
            eval(sprintf('save CT_XPA_sigma%1.4f_kub%1.2f_klb%1.2f_zm%1.2f_intK%d_intZ%d.mat gridK gridZ A3 Kdot XPA_K XPA_KK DH_Max DH_Mean',sigma,kub,klb,zm,intK,intZ));
        else % robustness for mu
            eval(sprintf('save CT_XPA_mu%1.2f_sigma%1.4f_kub%1.2f_klb%1.2f_zm%1.2f_intK%d_intZ%d.mat gridK gridZ A3 Kdot KS_K KS_KK DH_Max DH_Mean',mu,sigma,kub,klb,zm,intK,intZ));
        end
    end
