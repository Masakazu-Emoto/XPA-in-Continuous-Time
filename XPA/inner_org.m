function [Ass, Bss, WW, vss, cs, ps, zx, zy, zz] = inner_org(Kdot, vss, r, w)
...
%% OUTPUTS
% Ass   : A_lm
% Bss   : A_lm, excluding the effect of aggregate variables, to be used when we solve the KF equation
% Wss   : the transition matrix, A3
...
% We extend their original code in the following two dimensions:
...
% A_lm in their note when we solve the KF equation (there is the indirect effect through r and w).
    ...
    for n = 1:maxit
        ...
        elem_c       = -elem_a-elem_e; % for B matrix
